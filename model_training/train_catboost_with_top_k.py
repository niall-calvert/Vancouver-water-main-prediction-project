...
def top_k_precision_report(y_true, probabilities, label, k_values=None):
    """
    Reports how well the model ranks actual leaks near the top of the risk list.

    Precision@K:
      Of the top K highest-risk pipe-year rows, what fraction actually leaked?

    Recall@K:
      Of all actual leaks in this split, what fraction were found in the top K?
    """
    if k_values is None:
        k_values = [100, 250, 500, 1000, 2500, 5000, 10000]
    ...
    total_actual_leaks = int(ranking_df["actual"].sum())
    total_rows = len(ranking_df)
    baseline_leak_rate = total_actual_leaks / total_rows if total_rows > 0 else 0

    rows = []

    print(f"\n{label} Top-K precision diagnostic")
    print("=" * 50)
    print(f"Total rows: {total_rows:,}")
    print(f"Total actual leaks: {total_actual_leaks:,}")
    print(f"Baseline leak rate: {baseline_leak_rate:.4f}")
    ...
    for k in k_values:
        ...
        leaks_found = int(top_k["actual"].sum())
        precision_at_k = leaks_found / k if k > 0 else 0
        recall_at_k = leaks_found / total_actual_leaks if total_actual_leaks > 0 else 0
        lift_over_baseline = (
            precision_at_k / baseline_leak_rate
            if baseline_leak_rate > 0
            else float("nan")
        )
        ...
        print(
            f"Top {k:,}: "
            f"found {leaks_found:,} of {total_actual_leaks:,} leaks | "
            f"Precision@K={precision_at_k:.4f} | "
            f"Recall@K={recall_at_k:.4f} | "
            f"Lift={lift_over_baseline:.2f}x"
        )
...
def main():
    ...
    val_probabilities_eval, val_predictions, val_metrics = evaluate(
        model,
        X_val,
        y_val,
        "Validation results",
    ...
    test_probabilities_eval, test_predictions, test_metrics = evaluate(
        model,
        X_test,
        y_test,
        "Test results",
    ...
    val_top_k_df = top_k_precision_report(
        y_true=y_val,
        probabilities=val_probabilities_eval,
        label="Validation",
    )

    test_top_k_df = top_k_precision_report(
        y_true=y_test,
        probabilities=test_probabilities_eval,
        label="Test",
    )

    top_k_df = pd.concat([val_top_k_df, test_top_k_df], ignore_index=True)
    top_k_df.to_csv("catboost_top_k_precision_report.csv", index=False)

    print("\nSaved catboost_top_k_precision_report.csv")
